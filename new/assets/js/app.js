...
function adminApp() {
  return {
    ...
    // Section aktif untuk halaman data-table (basic / special / custom)
    activeSection: "basic",
    ...
    initApp() {
      ...
      // Ambil section awal dari hash URL, misal #special-table -> "special"
      const target = window.location.hash;
      if (target.includes("special")) this.activeSection = "special";
      else if (target.includes("custom")) this.activeSection = "custom";
      else this.activeSection = "basic";

      // Redraw Tabulator agar tetap tampil saat container baru terlihat
      this.$nextTick(() => this.$dispatch("section-changed"));
    ...
    // Ganti section tabel + scroll ke kontennya
    scrollToSection(id) {
      this.$nextTick(() => {
        document
          .getElementById(id)
